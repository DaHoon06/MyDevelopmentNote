module.exports = async({ req, res, next, mongo, projectId }) => {
    try {
        ...
        const ageQuotaData = await getAgeQuota(mongo,SNUM,LOCAL_NUM);
        await setQAgeQuotaData(ageQuotaData,sendData);

        // TOTAL QUOTA
        const genderQuotaData = await getGenderQuota(mongo,SNUM,LOCAL_NUM);
        await setGenderQuota(genderQuotaData,sendData);

        const q1_1Data = await getQ1_1Data(mongo,SNUM,LOCAL_NUM);
        await setQ1_1Data(q1_1Data,sendData);

        const q1_2Data = await getQ1_2Data(mongo,SNUM,LOCAL_NUM);
        await setQ1_2Data(q1_2Data,sendData);


        // Q2 DATA
        const [q2Data] = await getQ2Data(mongo,SNUM,LOCAL_NUM);
        await setQ2Data(q2Data,sendData);

        // Q3 DATA
        const [q3Data] = await getQ3Data(mongo,SNUM,LOCAL_NUM);
        ...
        // const { TITLE: surveyTitle} = questions;
...
//COMPLETE
async function getGenderQuota(mongo,SNUM,LOCAL_NUM){
...
//COMPLETE
async function getAgeQuota(mongo,SNUM,LOCAL_NUM){
    return await mongo.db('getData').collection('DATA_634').aggregate([
        {$match : { SQ1 : LOCAL_NUM }},
        {$group: {
                _id: '$SQ3_R',
...
//COMPLETE
async function getQ1_1Data(mongo,SNUM,LOCAL_NUM){
    return await mongo.db('getData').collection('DATA_634').aggregate([
        {$match : { SQ1 : LOCAL_NUM }},
        {$group: {
                _id: '$Q1_1',
...
//COMPLETE
async function getQ1_2Data(mongo,SNUM,LOCAL_NUM){
    return await mongo.db('getData').collection('DATA_634').aggregate([
        {$match : { SQ1 : LOCAL_NUM }},
        {$group: {
                _id: '$Q1_2',
...
//COMPLETE
async function getQ2Data(mongo,SNUM,LOCAL_NUM){
    return await mongo.db('getData').collection('DATA_634').aggregate([
        {$match : { SQ1 : LOCAL_NUM }},
        {
            $group : {
...
//COMPLETE
async function getQ3Data(mongo,SNUM,LOCAL_NUM){
    return await mongo.db('getData').collection('DATA_634').aggregate([
        {$match : { SQ1 : LOCAL_NUM }},
        {
            $group : {
...
async function setQ2Data(qData,sendData){
    for(let i=1; i<=8; i++){
        sendData.qDataQ2[i-1] = qData['Q2_'+i].toFixed(3);
    }
}
...
async function setQ3Data(qData,sendData){
    for(let i=1; i<=11; i++){
        sendData.qDataQ3[i-1] = qData['Q3_'+i].toFixed(3);
    }
}
...
async function setQ1_1Data(qData,sendData){
    ...
    for(let i=0; i<=qData.length-1; i++){
        ...
        sendData.qDataQ1_1[q1Index-1] = qData[i] && +qData[i].sum;
    }
}
...
async function setQ1_2Data(qData,sendData){
    for(let i=0; i<=qData.length-1; i++){
        const q1Index = qData[i] && +qData[i]._id;
        sendData.qDataQ1_2[q1Index-1] = qData[i] && +qData[i].sum;
    }
}
